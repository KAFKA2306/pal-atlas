...
const sources = [
  {
    id: "palworld-gg",
    title: "Palworld.gg Paldeck / Breeding Calculator",
    url: EN_URL,
    role: "297-entry catalog and Breeding Rank",
  },
  {
    id: "palworld-wiki",
    title: "Palworld Wiki — Breeding",
    url: "https://palworld.wiki.gg/wiki/Breeding",
    role: "normal formula and special-combination rule cross-check",
  },
  {
    id: "palworld-wiki-images",
    title: "Palworld Wiki — Pal icon files",
    url: "https://palworld.wiki.gg/wiki/Template:Icon",
    role: "external icon URL cross-check when available",
  },
  {
    id: "webp-proxy",
    title: "wsrv.nl image optimizer",
    url: "https://images.weserv.nl/",
    role: "external WebP delivery URL generated from each source image",
  },
  {
    id: "game8",
    title: "Game8 — Breeding Combos Calculator",
    url: "https://game8.co/games/Palworld/archives/440530",
    role: "breeding workflow and special-combination cross-check",
  },
  {
    id: "paldeck",
    title: "Paldeck",
    url: "https://www.paldeck.cc/breeding",
    role: "independent database cross-check",
  },
  {
    id: "official-news",
    title: "Pocketpair official news",
    url: "https://news.palworldgame.com/",
    role: "official release and news context",
  },
  {
    id: "official-game",
    title: "Pocketpair official Palworld site",
    url: "https://www.pocketpair.jp/en/games-en/palworld-en/",
    role: "official game and version context",
  },
  {
    id: "official-docs",
    title: "Pocketpair official server docs",
    url: "https://docs.palworldgame.com/",
    role: "official documentation entry point",
  },
];
...
function parsePage(html) {
  const pattern =
    /<div class="pal"><div class="container ([^"]+)">.*?<div class="elements">(.*?)<\/div><div class="image">.*?<img[^>]+alt="([^"]+)"[^>]+src="([^"]+)".*?<div class="name">\s*[^<]+.*?comb-rank">\(([^)]+)\)/gs;
  return [...html.matchAll(pattern)].map((match, index) => {
    ...
    return {
      ...
      rarity: Number(
        block.match(
          /<div class="(?:common|rare|epic) rarity"><div class="lv">(\d+)<\/div>/,
        )?.[1] ?? 0,
      ),
      rarityTier: match[1],
...
async function loadSourcePals(html) {
  const assets = [
    ...new Set(
      [...html.matchAll(/\/_nuxt\/[^"']+\.js/g)].map(
        (match) => new URL(match[0], EN_URL).href,
      ),
    ),
  ];
  const scripts = await Promise.all(
    assets.map(async (url) => ({ url, text: await (await fetch(url)).text() })),
  );
  const route = scripts.find(({ text }) =>
    text.includes("../data/pals/en.json"),
  );
  const dataAsset = route?.text.match(
    /\.\.\/data\/pals\/en\.json.*?import\("([^"]+\.js)"\)/s,
  )?.[1];
  if (!route || !dataAsset)
    throw new Error("Palworld.gg source data module was not found");
  const sourceResponse = await fetch(new URL(dataAsset, route.url));
  if (!sourceResponse.ok)
    throw new Error(
      `Palworld.gg source data fetch failed: ${sourceResponse.status}`,
    );
  ...
  try {
    ...
    return Object.values(sourceModule).filter(
      (pal) => pal?.name && Number.isFinite(pal.combiRank),
    );
  } finally {
...
async function loadWikiIconUrls(pals) {
  ...
  for (let index = 0; index < pals.length; index += 20) {
    const titles = pals
      ...
      .map((pal) => `File:${pal.name} icon.png`)
      .join("|");
    const url = `${WIKI_API}?action=query&format=json&titles=${encodeURIComponent(titles)}&prop=imageinfo&iiprop=url|mime`;
    let response;
    for (let attempt = 0; attempt < 3; attempt += 1) {
      response = await fetch(url, {
        headers: { "User-Agent": "pal-atlas-data-builder/0.1" },
      });
      if (response.status !== 429 || attempt === 2) break;
      await new Promise((resolve) => setTimeout(resolve, 1000 * (attempt + 1)));
    }
    if (!response.ok)
      throw new Error(`Palworld Wiki image API failed: ${response.status}`);
...
if (!enResponse.ok || !jaResponse.ok)
  throw new Error(
    `Source fetch failed: ${enResponse.status}/${jaResponse.status}`,
  );
const [enHtml, jaHtml] = await Promise.all([
  enResponse.text(),
  jaResponse.text(),
]);
...
if (english.length < 250 || japanese.length < 250)
  throw new Error(
    `Unexpected catalog size: ${english.length}/${japanese.length}`,
  );
if (sourcePals.length < 250)
  throw new Error(`Unexpected source data size: ${sourcePals.length}`);
...
const resolveSourceId = (id) =>
  currentBySlug.get(sourceById.get(id)?.slug.trim());
const comboRows = sourcePals.flatMap((owner) =>
  (owner.combos ?? []).map((combo) => ({ owner, combo })),
);
...
const special = comboRows.flatMap(({ owner, combo }) => {
  ...
  return [
    {
      id: `special::${key}`,
      child: child?.id ?? slug(sourceChild?.name ?? combo.child),
      childName: child?.nameEn ?? sourceChild?.name ?? combo.child,
      parentA: parentA?.id ?? slug(sourceParentA?.name ?? combo.a),
      parentAName: parentA?.nameEn ?? sourceParentA?.name ?? combo.a,
      parentB: parentB?.id ?? slug(sourceParentB?.name ?? combo.b),
      parentBName: parentB?.nameEn ?? sourceParentB?.name ?? combo.b,
      kind:
        comboChildren.get([combo.a, combo.b].sort().join("::"))?.size > 1
          ? "gender-specific / source combo"
          : "source combo",
...
const rankSorted = pals
  .filter((pal) => !pal.ignoreCombi)
  .sort(
    (a, b) =>
      a.breedingRank - b.breedingRank ||
...
for (let i = 0; i < pals.length; i += 1) {
  for (let j = i; j < pals.length; j += 1) {
    const a = pals[i];
    const b = pals[j];
    const intermediatePower = Math.floor(
      (a.breedingRank + b.breedingRank + 1) / 2,
    );
    const child = rankSorted.reduce((best, candidate) => {
      ...
      return distance < bestDistance ||
        (distance === bestDistance && candidate.sourceIndex < best.sourceIndex)
        ? candidate
        : best;
    }, rankSorted[0]);
    const row = {
...
const addChildOutput = (parentId, row) => {
  ...
  if (
    !existing ||
    row.kind === "special" ||
    (existing.kind !== "special" && row.delta < existing.delta)
  )
    outputs.set(key, row);
};
for (const row of special) {
  for (const [parent, other] of [
    [row.parentA, row.parentB],
    [row.parentB, row.parentA],
  ]) {
    addChildOutput(parent, {
      id: row.id,
      parent,
      otherParent: other,
      child: row.child,
      kind: "special",
      specialKind: row.kind,
    });
  }
}
for (const row of normal) {
  ...
  for (const [parent, other] of [
    ...
    addChildOutput(parent, {
      ...
      child: row.child,
      kind: "normal",
      intermediatePower: row.intermediatePower,
      delta: Math.abs(row.intermediatePower - child.breedingRank),
      rule: row.rule,
    });
  }
}
const children = Object.fromEntries(
  [...childrenByParent].map(([parent, rows]) => [
    parent,
    [...rows.values()].sort(
      (a, b) =>
        (a.kind === "special" ? -1 : 1) - (b.kind === "special" ? -1 : 1) ||
        (a.delta ?? 0) - (b.delta ?? 0) ||
...
const childrenUi = Object.fromEntries(
  Object.entries(children).map(([parent, rows]) => [
    parent,
    { total: rows.length, rows: rows.slice(0, 12) },
  ]),
);
const featuredNormal = Object.fromEntries(
  [...normalByChild.entries()].map(([child, rows]) => [
    child,
    rows
      .sort(
        (a, b) =>
          Math.abs(a.intermediatePower - palsById.get(child).breedingRank) -
            Math.abs(b.intermediatePower - palsById.get(child).breedingRank) ||
          a.parentA.localeCompare(b.parentA),
      )
      .slice(0, 4),
  ]),
);
...
const meta = {
  ...
  formula:
    "floor((parentA.breedingRank + parentB.breedingRank + 1) / 2), then nearest eligible rank; source combos override",
...
await writeFile(
  `${DATA_DIR}/pals.json`,
  JSON.stringify({ meta, pals }, null, 2) + "\n",
);
await writeFile(
  `${DATA_DIR}/breeding.json`,
  JSON.stringify(
    { meta, normal, special, featuredNormal, normalCount: normal.length },
    null,
    2,
  ) + "\n",
);
await writeFile(
  `${DATA_DIR}/children.json`,
  JSON.stringify({ generatedAt: fetchedAt, outputs: children }, null, 2) + "\n",
);
await writeFile(
  `${DATA_DIR}/children-ui.json`,
  JSON.stringify({ generatedAt: fetchedAt, outputs: childrenUi }, null, 2) +
    "\n",
);
await writeFile(
  `${DATA_DIR}/sources.json`,
  JSON.stringify({ generatedAt: fetchedAt, sources }, null, 2) + "\n",
);

const normalRows = normal.map((row) => ({
  ...row,
  parentAName: palsById.get(row.parentA).nameEn,
  parentBName: palsById.get(row.parentB).nameEn,
  childName: palsById.get(row.child).nameEn,
}));
await writeFile(
  `${NEO4J_DIR}/pals.csv`,
  csv(
    pals.map((pal) => ({ ...pal, elements: pal.elements.join("|") })),
    [
      "id",
      "order",
      "nameEn",
...
console.log(
  `Generated ${pals.length} pals, ${normal.length} normal pairs, ${special.length} special rows (${special.filter((row) => row.status === "resolved").length} resolved).`,
);
