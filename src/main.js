...
const copy = {
  ja: {
    ...
    formula: "配合ルール",
    formulaText:
      "親2体の配合値から中間値を出し、最も近い配合値のパルへ着地します。特殊配合はこのルールを上書きします。",
    direct: "固有レシピ",
    nearby: "値が近い親",
    unresolved: "未解決の出典行",
    cards: "パル",
    noResult: "該当するパルがありません。",
  },
  en: {
    routes: "Breeding routes",
    saved: "Saved",
    search: "Search Pals",
    all: "All",
    parents: "How to make",
    children: "Make from this Pal",
    outputs: "Outputs",
    more: "Show all",
    special: "Special",
    normal: "Normal",
    ...
    choose: "Choose a Pal",
    formula: "Normal breeding rule",
    formulaText:
      "The two parent ranks become an intermediate value, then resolve to the closest eligible Pal rank. Special combinations override this rule.",
    direct: "Exact recipe",
    nearby: "Rank-near parent",
    unresolved: "Unresolved source row",
    cards: "Pals",
    noResult: "No Pals match this filter.",
  },
};
...
function readSavedRecipes() {
  const value = JSON.parse(
    localStorage.getItem("pal-atlas-saved-recipes") ?? "[]",
  );
  return new Map(
    Array.isArray(value)
      ? value
          .filter(
            (row) =>
              row?.id &&
              byId.has(row.parentA) &&
              byId.has(row.parentB) &&
              byId.has(row.child),
          )
          .map((row) => [row.id, row])
      : [],
...
const elText = (element) =>
  elementLabels[element]?.[state.lang === "ja" ? 0 : 1] ?? element;
...
function imageMarkup(pal, alt = "") {
  ...
  const fallback =
    primary !== original ? ` data-fallback="${esc(original)}"` : "";
  return `<img src="${esc(primary)}"${fallback} alt="${esc(alt)}" loading="lazy" />`;
}

function directParentRows(pal) {
  return special
    .filter((row) => row.child === pal.id)
    .map((row) => ({
      id: row.id,
      kind: "special",
      parentA: byId.get(row.parentA),
      parentB: byId.get(row.parentB),
      child: pal.id,
      label: row.kind,
    }));
}
...
function normalParentRows(pal) {
  return (featuredNormal[pal.id] ?? []).map((row) => ({
    ...
    delta: Math.abs(row.intermediatePower - pal.breedingRank),
    label: t("nearby"),
  }));
}
...
function parentRows(pal) {
  ...
  const normal = normalParentRows(pal).filter(
    (row) =>
      !direct.some(
        (item) =>
          item.parentA?.id === row.parentA?.id &&
          item.parentB?.id === row.parentB?.id,
      ),
  );
...
function recipeRecord(row) {
  ...
  return {
    id: row.id,
    parentA: id(row.parentA ?? row.parent),
    parentB: id(row.parentB ?? row.otherParent),
    child: id(row.child ?? row.childPal),
    kind: row.kind,
    intermediatePower: row.intermediatePower ?? null,
  };
}
...
function outputRows(pal) {
  return (outputCache.get(pal.id) ?? outputs[pal.id]?.rows ?? [])
    .map((row) => ({
      ...row,
...
function savedSection(rows) {
  ...
  return `<section class="saved-section" id="saved-view"><div class="section-head"><div><span class="micro-label">05 / ${t("saved")}</span><h2>${t("savedRecipes")}</h2></div><span class="result-count">${rows.length}</span></div><div class="saved-list">${rows
    .map((row) => {
      const parentA = byId.get(row.parentA);
      const parentB = byId.get(row.parentB);
...
function filteredPals() {
  ...
  return pals
    .filter(
      (item) =>
        `${item.nameEn} ${item.nameJa}`.toLowerCase().includes(query) &&
        (state.element === "all" || item.elements.includes(state.element)),
    )
...
function catalogState() {
  ...
  const visible =
    state.showAllPals || isFiltered
      ? items
      : items.slice(0, DEFAULT_CATALOG_LIMIT);
  return {
    items,
    visible,
    expandable:
      !state.showAllPals && !isFiltered && items.length > visible.length,
  };
}
...
function updateCatalog() {
  ...
  if (resultCount)
    resultCount.textContent = `${catalog.visible.length} / ${catalog.items.length}`;
...
function recipePal(pal, role) {
  ...
  const action =
    role === "target"
      ? ""
      : ` data-select="${pal.id}" data-drill="true" aria-label="${esc(palName(pal))}"`;
...
function outputView(pal) {
  ...
  const more =
    !state.showAllOutputs && outputCount(pal) > visible.length
      ? `<button class="more-button" data-more-outputs="true">${t("more")} / ${outputCount(pal)}</button>`
      : "";
...
function insight(pal) {
  ...
  if (direct.length)
    return `<div class="insight"><span class="signal-mark">✦</span><div><b>${t("direct")}</b><p>${state.lang === "ja" ? "この親の組み合わせは配合値の近さではなく、固有レシピが結果を決めます。" : "This parent pair is an exact recipe; it wins over rank-nearest calculation."}</p></div></div>`;
  const delta = rows[0]
    ? Math.abs(rows[0].intermediatePower - pal.breedingRank)
    : 0;
...
function render() {
  ...
  const catalog = catalogState();
  const allElements = [
    ...new Set(pals.flatMap((item) => item.elements)),
  ].sort();
...
function bind() {
  ...
  app.addEventListener("click", async (event) => {
    const target = event.target.closest(
      "[data-select], [data-back], [data-save-recipe], [data-saved-link], [data-more-pals], [data-more-outputs], [data-lang-toggle], [data-theme-toggle]",
    );
    ...
    if (target.dataset.saveRecipe) {
      ...
      if (
        !recipe.id ||
        !byId.has(recipe.parentA) ||
        !byId.has(recipe.parentB) ||
        !byId.has(recipe.child)
      )
        return;
      if (savedRecipeIds.has(recipe.id)) savedRecipeIds.delete(recipe.id);
      else savedRecipeIds.set(recipe.id, recipe);
      writePreference(
        "pal-atlas-saved-recipes",
        JSON.stringify([...savedRecipeIds.values()]),
      );
    ...
    if (target.dataset.moreOutputs !== undefined) {
      const detail = await (
        await fetch(`./api/pals/${selected().id}.json`)
      ).json();
    ...
    if (target.dataset.select) {
      ...
      state.stack =
        target.dataset.drill === "true" ? [...state.stack, id] : [id];
  ...
  app.addEventListener(
    ...
    (event) => {
      ...
      const fallback =
        image instanceof HTMLImageElement ? image.dataset.fallback : "";
