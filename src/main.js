...
function outputRows(pal) {
  return (outputs[pal.id]?.rows ?? []).map((row) => ({
    ...row,
  ...
  document.querySelector('#app').innerHTML = `<main class="shell">
    ...
    <section class="workspace" id="atlas-view">
      ...
      <aside class="detail-column"><div class="detail-card"><div class="trail"><button data-back ${state.stack.length > 1 ? '' : 'disabled'}>←</button><span>${state.stack.map((id) => esc(palName(byId.get(id)))).join(' <i>→</i> ')}</span></div><div class="selected-portrait">${imageMarkup(pal, palName(pal))}</div><div class="selected-title"><span class="micro-label">#${String(pal.order).padStart(3, '0')} / ${pal.rarityTier}</span><h2>${esc(palName(pal))}</h2><p>${pal.nameEn === pal.nameJa ? '' : esc(state.lang === 'ja' ? pal.nameEn : pal.nameJa)}</p></div><div class="tag-row">${pal.elements.map((element) => `<span class="tag element-${element}">${elText(element)}</span>`).join('')}<span class="tag rank-tag">${t('rank')} ${pal.breedingRank}</span></div>
        <div class="divider"></div><div class="detail-label">${t('insight')}</div>${insight(pal)}<div class="detail-label">${t('parents')}</div><div class="parent-list">${parentRows(pal).slice(0, 4).map((row) => `<div class="parent-row ${row.kind}"><span class="pair-images">${row.parentA ? `<button data-select="${row.parentA.id}" data-drill="true" aria-label="${esc(palName(row.parentA))}">${imageMarkup(row.parentA)}</button>` : ''}${row.parentB ? `<button data-select="${row.parentB.id}" data-drill="true" aria-label="${esc(palName(row.parentB))}">${imageMarkup(row.parentB)}</button>` : ''}</span><span><b>${row.parentA ? esc(palName(row.parentA)) : '—'} + ${row.parentB ? esc(palName(row.parentB)) : '—'}</b><small>${row.kind === 'special' ? t('direct') : `${t('nearby')} / ${row.intermediatePower}`}</small></span><span class="arrow">→</span></div>`).join('') || `<p class="muted">${t('unresolved')}</p>`}</div></div>
